...
Каталог из 67 упражнений собран по оборудованию типового зала: семь групп мышц, у ходовых движений есть иллюстрации. Тренировки принадлежат пользователю, длина цикла равна их числу — три тренировки или шесть, всё сойдётся.

Конструктор открывается с экрана «Тренировка»: «Изменить» правит текущую, «Своя тренировка» создаёт новую. Внутри — поиск по каталогу с фильтром по группе мышц, подходы, диапазон повторов и рабочий вес для каждого упражнения, порядок стрелками. Если движения нет в каталоге, пустой поиск предложит добавить своё — оно останется приватным. Кардио в каталог не входит: оно не описывается подходами и повторами, для него есть отдельная запись с дистанцией и темпом.

Статические упражнения меряются секундами: у планки вместо счётчика повторов секундомер. Своему упражнению единицу можно выбрать при создании.

Тап по упражнению в плане открывает историю: график рабочего веса по тренировкам и личные рекорды — максимальный вес, повторы и лучший объём за тренировку. При завершении тренировки приложение показывает, где именно ты побил предыдущий результат.
